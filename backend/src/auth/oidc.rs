...
use crate::{auth::jwt_state::JwtState, config::Config, db::DBTrait, rate_limit::RateLimiter};
...
pub struct OidcState(Arc<Mutex<OidcConfig>>);
...
impl OidcState {
  pub async fn new(settings: &Config) -> Self {
    let state = Self(Arc::new(Mutex::new(
      OidcConfig::new(settings)
        .await
        .expect("Failed to init OIDC config"),
    )));
...
impl OidcConfig {
  async fn new(settings: &Config) -> Result<Self> {
    let mut url = settings.oidc_issuer.clone();
    url
      .path_segments_mut()
      .ok()
    ...
    Ok(Self {
      ...
      client,
      scope: settings.oidc_scopes.clone(),
...
async fn oidc_url(
  ...
  if !config.scope.is_empty() {
    form.insert("scope", config.scope.clone());
  }
...
async fn oidc_callback(
  ...
  let mut config = oidc_state.0.lock().await;
